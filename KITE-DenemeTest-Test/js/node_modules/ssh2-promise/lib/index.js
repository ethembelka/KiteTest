...
const sshConnection_1 = __importDefault(require("./sshConnection"));
...
const BaseSSH2Promise_1 = __importDefault(require("./BaseSSH2Promise"));
        ...
        close: function () {
            ...
            if (sshConnection.__sshTunnels.length > 0) {
                ...
                if (sshConnection.config.sock) {
                    sshConnection.config.sock.destroy();
                }
            ...
            ((sshConfig, isLast) => {
                if (!lastSSH) {
                    ...
                    lastSSH = lastSSH.then((ssh) => {
                        if (ssh.config.hoppingTool === sshConstants_1.default.HOPPINGTOOL.SOCAT) {
                            return ssh.spawn(`socat - TCP:${sshConfig.host}:${sshConfig.port}`);
                        }
                        else if (ssh.config.hoppingTool === sshConstants_1.default.HOPPINGTOOL.NATIVE) {
                            return ssh.forwardOut('127.0.0.1', sshUtils_1.default.getRandomPort(), sshConfig.host, sshConfig.port);
                        }
                        else {
